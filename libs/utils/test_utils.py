...
class Transform:
    ...
    def __call__(self, raw_img, mask_img):
        ...
        mix_img = maskAddImg_mul(raw_img, mask_img)
        ...
        return dict(mask_img=mask_img_n, mix_img=mix_img, raw_img=raw_img, person=img_person, back=img_back)


def maskAddImg_mul_25percent(img, mask):
    '''
    bgr
    1 : 烤烟，蓝色（255,0,0），
    2：玉米，黄色（0,255,255），
    ...
    black = cv2.resize(black, (0,0),fx=1/4., fy=1/4., interpolation=cv2.INTER_NEAREST)
    black = cv2.resize(black, (0,0),fx=4., fy=4., interpolation=cv2.INTER_NEAREST)
