...
class KLDivLoss_wapper(object):
    ...
    def __call__(self, x, y):
        ...
        y = F.softmax(y, 1)
        return self.KL(log_x, y)


class TSLoss(object):
    def __init__(self, recon_weights, CE_weights, KL_weights, mode, is_Norm=False, ignore_index=255):
        self.mode = mode
        if self.mode == "reconstruction":
            self.reconstruction = nn.MSELoss(reduction='sum')
        elif self.mode == "KLDiv":
            self.KL = KLDivLoss_wapper()
        elif self.mode == "reconstruction&KLDiv":
            self.reconstruction = nn.MSELoss(reduction='sum')
            self.KL = KLDivLoss_wapper()
        self.criterion = nn.CrossEntropyLoss(ignore_index=ignore_index, )
    ...
    def __call__(self, output, target):
        # assert len(output) == 3
        bs = target.size(0)
        HW = target.size(2) * target.size(1)
        if self.is_Norm:
            if self.mode == "reconstruction":
                loss_recon = self.reconstruction(output[0], output[1]) / float(bs) / float(HW) * self.recon_weights
            elif self.mode == "KLDiv":  # t_result, s_result
                loss_recon = self.KL(output[1], output[0]) / float(bs) / float(HW) * self.KL_weights
            elif self.mode == "reconstruction&KLDiv":
                loss_recon = self.reconstruction(output[0], output[1]) / float(bs) / float(HW) * self.recon_weights + \
                self.KL(output[1], output[0]) / float(bs) / float(HW) * self.KL_weights
        else:
            if self.mode == "reconstruction":
                loss_recon = self.reconstruction(output[0], output[1]) / float(bs) * self.recon_weights
            elif self.mode == "KLDiv":  # t_result, s_result
                loss_recon = self.KL(output[1], output[0]) / float(bs) * self.KL_weights
            elif self.mode == "reconstruction&KLDiv":
                loss_recon = self.reconstruction(output[0], output[1]) / float(bs) * self.recon_weights + \
                self.KL(output[-1], output[-2]) / float(bs) * self.KL_weights
                return loss_recon

        if len(output) == 2:
            loss_target = self.criterion(output[1], target) / float(bs)
        elif len(output) == 3:
            loss_target = self.criterion(output[2], target) / float(bs)
        else:
            raise RuntimeError("model wrong output")
        return loss_recon + loss_target * self.CE_weights
...
class Lovasz_focal2_loss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        return self.focal_weight * self.focal_loss(inputs_softmax, targets) + self.lovasz_weight * lovasz_softmax(
            inputs_softmax,
            targets)
...
class Lovasz_focal_loss(nn.Module):
    ...
    def forward(self, inputs, targets):
        return self.focal_weight * self.focal_loss(inputs, targets) + self.lovasz_weight * self.lovasz_loss(inputs,
                                                                                                            targets)
